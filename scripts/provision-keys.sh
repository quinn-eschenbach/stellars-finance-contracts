...
# Persist a network-scoped env snapshot so server bundles don't have to call
# the CLI at runtime. The deploy script later overwrites this with the full
# service env; provision-keys writes only the identity block so re-runs are
# safe.
echo ""
echo "=== Writing $ENV_FILE (identity block) ==="
{
  echo "# Auto-generated by scripts/provision-keys.sh — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
