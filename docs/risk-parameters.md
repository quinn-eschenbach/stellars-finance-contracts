...
- Oracle prices are scaled by `PRECISION = 10_000_000` (1e7). `PRECISION` is NOT the USDC token scale — it is the price scale only.
...
- Raise `tp_sl_execution_fee` (cap 100_000 USDC at 6 decimals) — but note this is a flat fee paid up front by the trader.
